(function (paper) {
  ...
  function adaptiveOffsetCurve(curve, offset) {
      ...
      // console.warn(hNormal);
      // var hNormal = curve.getNormalAtTime(0.5).multiply(offset);
  ...
  function makeRoundCap(from, to, offset) {
      ...
      // console.error(from.point, to.point);
  ...
  function drawCircle(point,fill){
    const circle = document.createElementNS("http://www.w3.org/2000/svg", "circle");
    circle.setAttribute("cx", point._x);
    circle.setAttribute("cy", point._y);
    circle.setAttribute("r", 5);
    circle.setAttribute("fill", fill);
    document.querySelector("svg #grid").appendChild(circle);
  }
  ...
  function connectSide(outer, inner, offset, cap) {
      ...
      switch (cap) {
          case 'round':
              ...
              // drawCircle(iSegments[iSegments.length - 1].point,"red");
